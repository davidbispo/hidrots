...
Description: Program for time series analysis. It has 3 modules
    ...
    4) sep_stright: Prints on a pre-loaded matplotlib figure the baseflow hydrograph using the straight 
       line method. It uses a nx2 dataframe. Series may have any timestep.
...
Prompt : Time series printing 1.0
What is the file adress? No need for quotes. hidrots reads as string already
*Plase notice that python does not handles well long addresses or cloud 
address files on windows 
...
def limit_parser(filename):

    limits = raw_input("""
Provide a list with inferior and superior limits on Python datetime.date format. 
*Ex: [datetime.date(2003, 1, 1),datetime.date(2017, 12, 31)]
**Please notice that this file is an outer limit for the runoff separation method
...
def header_parser():
        ...
        header = str(raw_input("""
Please provide the index collumn name date. No need for quotes. hidrots reads as string already
                                
Ans[AsRaw]: """))
        indexparser = [header]
        
        return header, indexparser
        
def df_parser (filename, header, indexparser, limits):
        data_df = getts (filename = filename, index_col = header, indexparse = indexparser, separator = ',', limits= limits)
        timeseries_list = list(data_df)
        return data_df, timeseries_list        
        
def rf_parser(timeseries_list):
    
        for i in range(len(timeseries_list)):
            print i , '-' ,timeseries_list[i], '\n'########
        ans_select_rain = raw_input("""
Which among these are the rainfall series series would you like to plot 
Please provide a list
                    
Ans[AsList] : """)              
        ans_select_rain = eval(ans_select_rain)
        
        rainlist = []
        for item in ans_select_rain:
            rainlist.append(timeseries_list[item])
        return rainlist

def fl_parser(timeseries_list, type_ans):
    if type_ans == 3:
        k = 0
        for i in timeseries_list:
            print '%d - %s' % (k, i)     
    
    ans_select_flow = raw_input("""
Which among these are the flow series series would you like to plot 
Please provide a list for time series printing or an integer for runoff
separation:
                    
Ans[AsList] : """)
    if type_ans == 1 or type_ans ==2:
        ans_select_flow = eval(ans_select_flow)
        flowlist = []
          
        for item in (ans_select_flow):
            flowlist.append(timeseries_list[item])
    else:
        ans_select_flow = int(ans_select_flow)
        flowlist = timeseries_list[ans_select_flow]
        
        type_ans != 2
    ans_select_output = raw_input("""
Would you like an output? Case positive prompt the output adress
Case negative just respond "N"

Ans[AsRaw] : """)
    return flowlist, ans_select_output
...
def hsprinter (data_df, flow_list, rainfall_list, output = '', print_tofile = True, dpi = 600, size = (30,10)):
    ...
    for index in range(len(flow_list)):
        ax1.plot_date(x = datelist , y = flow[index], linestyle='solid', marker='None', 
                      label = flow_list[index], xdate = True, color = colorflow[index] , linewidth=1.0, alpha=0.7)
    ...
    for index in range(len(rainfall_list)):
        ax2.plot_date(x = datelist , y = rfall[index], linestyle='solid', marker='None', 
                      label = rainfall_list[index], xdate = True, color = colorrfall[index], linewidth=1.0, alpha=0.7)
    ax2.legend()
...
def hsprint_postprint(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output):
     ans_select_print = int(raw_input("""
...
Ans[AsRawInteger]: """))
     if ans_select_print == 1 :
        limits = limit_parser(filename)
        data_df, timeseries_list = df_parser(filename, header, indexparser, limits)
        print_launcher(data_df,flowlist, rainlist, output = ans_select_output)
        hsprint_postprint(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output)
        
     elif ans_select_print == 2 :
        rainlist = rf_parser(timeseries_list)
        print_launcher(data_df,flowlist, rainlist, output = ans_select_output)
        hsprint_postprint(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output)
        
     elif ans_select_print == 3 :
        flowlist = fl_parser(timeseries_list)[0]
        print_launcher(data_df,flowlist, rainlist, output = ans_select_output)
        hsprint_postprint(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output)
           
#  hsprinter Launcher   
def print_launcher(data_df,flowlist, rainlist, output = ans_select_output, print_tofile = True):    
    if ans_select_output !="N":
        hsprinter(data_df,flowlist, rainlist, output = ans_select_output, print_tofile = True)    
    elif ans_select_output == "N":
        hsprinter(data_df,flowlist, rainlist, output = None, print_tofile = False)              
...
def missingreport (data_df, flow_list, rainfall_list, output, printfig = True, output_flaw = '', dpi = 600, figsize=(20,10)):
    ...
    fig_limit = data_df.index.tolist()
    fig_limits_lower = fig_limit[0]
    fig_limits_upper = fig_limit[-1]
    ax.get_yaxis().set_visible(False)          
    ax.grid(b=True, which = 'major', linewidth = 0.7)
    ax.grid(b=True, which = 'minor', linewidth = 0.1, color = 'black')
    plt.title('Flaw report', y = 1.0, fontsize=20)
    plt.legend(loc='lower left', bbox_to_anchor=(-0.130,0))
    major_tick_list = pd.date_range(fig_limits_lower, fig_limits_upper, freq = '1Y').tolist()
    minor_tick_list = pd.date_range(fig_limits_lower, fig_limits_upper, freq = '12W').tolist()      
    myFmt = matplotlib.dates.DateFormatter('')
    ax.xaxis.set_minor_formatter(myFmt)
    ax.set_xticks(major_tick_list)
    ax.set_xticks(minor_tick_list, minor = True)
    #plt.setp(plt.gca().xaxis.get_minorticklabels(),'rotation', 90)
    plt.grid(b=True, which='major', color='b', linestyle='-')
    plt.grid(b=True, which='minor', color='gray', linestyle='-')
    plt.xlim(fig_limits_lower, fig_limits_upper)
...
#missingreport DURATION PARSER    
def event_duration_parser():
    ans_duration = raw_input("""
Please provide a list with start and end of an event on the datetime.date format
*Ex: [datetime.date(2003, 1, 1),datetime.date(2017, 12, 31)]
**Please notice that this file is an INNER limit for the runoff separation method, 
so that you can see the line created for runoff separation 

Ans[AsDatetimeList] : """)
        
    ans_duration = eval(ans_duration)
    return ans_duration

#missingreport LAUNCHER
def print_launcher_flaws(data_df, flowlist, rainlist,ans_select_output):
    
    if ans_select_output !="N":
        missingreport (data_df, flowlist, rainlist, output = ans_select_output, printfig = True)
    elif ans_select_output == "N":
        missingreport (data_df, flowlist, rainlist, output = None, printfig = False)
    
######################################RUNOFF SEP - STRAIGHT LINE METHOD##########################
#sep_straight MODULE
def sep_straight(data_df, event_datelist, flow_name, output):
    """
...
#PLOTTING#
    ...
    print "This event had a total runoff of %.3d cms" % (vol)
#sep_straight TEST
...
def print_select_sep():
     ans_print = int(raw_input("""                    
Would you like an ouput? Type adress or N
Ans[AsRaw]: """))
     return ans_print
     
def launch_sep_straight(ans_select_output, data_df, event_datelist, flowlist):
     if ans_select_output == "N":
         sep_straight(data_df, event_datelist, flowlist, output="N")
     elif ans_select_output != "N":
         sep_straight(data_df, event_datelist, flowlist, output=ans_select_output)
############################   MAIN MENU    #################################################  
def main_menu():
    ans_main = int(raw_input("""
What would you like to do?)

  1 - Print a series(active)
  2 - Get a missing data report on a hydrological time series(active)
  3 - Separate runoff in a timeseries(active)
  4 - Perform hydrological scenarios analysis(not active)
  5 - Get statistics from a hydrological time series(not active)
  6 - Get a ARIMA synthetic series from an annual series(not active)
...
if ans_main == 1:
    ...
    limits = limit_parser(filename)
    header, indexparser = header_parser()
    data_df, timeseries_list = df_parser(filename, header, indexparser, limits)
    rainlist = rf_parser(timeseries_list)
    flowlist, ans_select_output = fl_parser(timeseries_list,ans_main)
    print_launcher(data_df,flowlist, rainlist, output = ans_select_output)
    hsprint_postprint(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output)
elif ans_main == 2:
    filename = file_parser()
    ...
    rainlist = rf_parser(timeseries_list)
    flowlist, ans_select_output = fl_parser(timeseries_list, ans_main)
    print_launcher_flaws(data_df, flowlist, rainlist,ans_select_output)     
elif ans_main == 3:
    filename = file_parser()
    limits = limit_parser(filename)
    event_datelist = event_duration_parser()
    header, indexparser = header_parser()
    data_df, timeseries_list = df_parser(filename, header, indexparser, limits)
    flowlist, ans_select_output = fl_parser(timeseries_list, ans_main)
    launch_sep_straight(ans_select_output, data_df, event_datelist, flowlist)  