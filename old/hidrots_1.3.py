...
def limit_parser(filename):
    ...
    limits = raw_input("""
Prompt : Provide a list with inferior and 
superior limits in datetime format
...
Prompt : Please provide a string with the index collumn name date
...
Please provide a list with start and end of an event
                    
Answer : """)
        
    ans_duration = eval(ans_duration)
    return ans_duration
#missingreport OUTPUT PARSER
def print_select_flaws():
     ans_print_out = raw_input("""
Would you like an output? Case positive prompt the output adress
Case negative just respond "N"
                                
Answer : """)
     return ans_print_out
#missingreport LAUNCHER
def print_launcher_flaws(data_df, flowlist, rainlist,output):
    
    if ans_select_output !="N":
        missingreport (data_df, flowlist, rainlist, output = ans_print, printfig = True)
    elif ans_select_output == "N":
...
What would you like to do?)
  ...
  2 - Get a missing data report on a hydrological time series
  3 - Separate runoff in a timeseries
  4 - Perform hydrological scenarios analysis(not active)
  5 - Get statistics from a hydrological time series(not active)
  6 - Get a ARIMA synthetic series(not active)
...
if ans_main == 1:
    ...
    data_df, timeseries_list = df_parser(filename, header, indexparser, limits)
    rainlist = rf_parser(timeseries_list)
    flowlist, ans_select_output = fl_parser(timeseries_list)
    print_launcher(data_df,flowlist, rainlist, output = ans_select_output)
    hsprint_postprint(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output)
    ...
    rainlist = rf_parser(timeseries_list)
    flowlist, ans_select_output = fl_parser(timeseries_list)
    ans_print = print_select_flaws()
    print_launcher_flaws(data_df, flowlist, rainlist,output = ans_print)     
elif ans_main == 3:
    filename = file_parser()
    limits = limit_parser(filename)
