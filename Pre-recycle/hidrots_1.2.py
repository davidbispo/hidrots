...
def missingreport (data_df, flow_list, rainfall_list, output = '', printfig = True, output_flaw = '', dpi = 600, figsize=(20,10)):
    ...
    #date_interval = fig_limits_upper - fig_limits_lower
...
############################   INPUT MODULES - printer    ############################################
  
def main_menu():
    ans_main = int(raw_input("""What would you like to do?)
                    1 - Print a series(active)
                    2 - Get a missing data report on a hydrological time series
                    3 - Separate runoff in a timeseries(not active)
...
def file_parser():
    if ans_main == 1:
        try:
            filename = str(raw_input("""Prompt : Time series printing 1.0
...
def limit_parser(filename):
        ...
        limits = raw_input("""Prompt : Provide a list with inferior and 
...
def header_parser():
        ...
        header = str(raw_input("""Prompt : Please provide a string with 
                                the index collumn name date
...
def rf_parser(timeseries_list):
        ...
        ans_select_rain = raw_input("""Which among these are the rainfall series series would you like to plot 
...
def fl_parser(timeseries_list):
    
        ans_select_flow = raw_input("""Which among these are the flow series series would you like to plot 
                                          Please provide a list?
        ...
        ans_select_output = raw_input("""Would you like an output? Case positive prompt the output adress
...
def print_launcher(data_df,flowlist, rainlist, output = ans_select_output, print_tofile = True):
        ...
        if ans_select_output !="N":
            print_option = hsprinter(data_df,flowlist, rainlist, output = ans_select_output, print_tofile = True)    
        elif ans_select_output == "N":
            print_option = hsprinter(data_df,flowlist, rainlist, output = None, print_tofile = False) 
        
        
def post_print(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output):
     ans_select_print = int(raw_input("""Which input would you like to change
                                 1 - Printing limits
                                 2 - Rainfall list
                                 3 - Flow list
     ...
     if ans_select_print == 1 :
        ...
        post_print(filename,data_df, header, timeseries_list,rainlist,flowlist,ans_select_output)
...
############################   INPUT MODULES - flawreport    ############################################
    
def event_duration_parser():
    ans_duration = raw_input("""Please provide a list with start and end of an event
...
def print_select_flaws():
     ans_print_out = raw_input("""Would you like an output? Case positive prompt the output adress
                                Case negative just respond "N"
                                
                                Answer : """)
     return ans_print_out
 
def print_launcher_flaws(data_df, flowlist, rainlist,output = ans_print):
    
    if ans_select_output !="N":
            print_option = missingreport (data_df, flow_list = flowlist, rainfall_list=rflist, output = ans_print, printfig = True)
        elif ans_select_output == "N":
            print_option = missingreport (data_df, flow_list = flowlist, rainfall_list=rflist, output = None, printfig = False)
...
if ans_main == 2:
    ...
    filename = file_parser()
    limits = limit_parser(filename)
    event_duration = event_duration_parser()
    header, indexparser = header_parser()
    data_df, timeseries_list = df_parser(filename, header, indexparser, limits)
    rainlist = rf_parser(timeseries_list)
    flowlist, ans_select_output = fl_parser(timeseries_list)
    ...
    #[datetime.datetime(2016, 8, 17, 13),datetime.datetime(2016, 8, 28, 11)]
