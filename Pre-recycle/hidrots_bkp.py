...
def getts (filename, index_col, indexparse, separator, limits):
    ...
    Module arguments are:
        ...
        """    
    try:
        df = pd.read_csv(filename, index_col = index_col, sep = separator , parse_dates = indexparse, infer_datetime_format = True)
...
def test_getts():
    ...
    df = getts (filename = filepath, index_col = 'Datahora', indexparse = ['Datahora'], separator = ',', limits= limits)
    item = df.loc[datetime.date(2012,6,18)]
...
def hsprinter (df, flow_list, rainfall_list, output = '', print_tofile = True, dpi = 600, size = (30,10)):
...
def test_hsprinter():
    ...
    rflist = ['2549107']
    df = getts(fileinput, index_col = 'Datahora', indexparse = ['Datahora'], separator = ',', limits = limits)
    hsprinter (df, flowlist, rflist, output = output, print_tofile = True, dpi = 600, size = (30,10))
...
def missingreport (df, flow_list, rainfall_list, output = '', printfig = True, output_flaw = '', dpi = 600, figsize=(20,10)):
    ...
    Arguments:
        df : A pandas dataframe(refer to getts() function)
...
def test_missingreport():
    ...
    df = getts(fileinput_sep, index_col = 'Datahora', separator = ',', indexparse = ['Datahora'], limits = limits) 
    output = os.path.join(test_folder,'output_flaw.jpg')
    missingreport(df,flowlist,rflist,output = output,printfig = True, dpi = 300)
    success = os.path.isfile(output) == True
...
def sep_straight(df, event_datelist, FLOWFieldName, output):
    """
...
#BASIC COMPUTATIONS FOR THE STRAIGHT LINE METHOD#
    df_limited = df.loc['%s'%(event_datelist[0]):'%s'%(event_datelist[1])]
    flow_start_row = df_limited.iloc[0]
    flow_start = flow_start_row['%s' % (FLOWFieldName)]
    flow_end_row = df_limited.iloc[-1]
    flow_end = flow_end_row['%s' % (FLOWFieldName)]
    delta_t = event_datelist[1]-event_datelist[0]
    delta_t = delta_t.total_seconds()
    
##STRAIGHT LINE METHOD#
    flow_obs_lim = df_limited['%s' % (FLOWFieldName)].tolist()
...
#PLOTTING#
    ...
    fig = plt.figure(figsize = (30,10))
    ax = plt.gca()
    flow_obs = df[FLOWFieldName].tolist()
    datelist_sub = df_limited.index.tolist()
...
def test_sep_straight():
    ...
    df = getts(fileinput_str, index_col = 'Datahora', indexparse = ['Datahora'], separator = ',', limits = limits)
    calculated = sep_straight(df,event_duration, FLOWFieldName = 'Vazao', output = saida)
    expected = 24909909
...
#limits = [datetime.date(2000, 1, 1),datetime.date(2016, 12, 31)]
#df = getts (filename = filename, index_col = 'Datahora', indexparse = ['Datahora'], separator = ',', limits= limits)
#test_getts()

#flowlist = ['65019700_cax', '65019675_stq', 'Tmd_vaz']
#rflist = ['2549077' , '2549080', '2549081','2549100','Simepar', '2549107']
#hsprinter(df,flowlist, rflist, output = r'C:\Users\david\Desktop\hidrots_1.0\test\hs_print.png', print_tofile = True, dpi = 600)
#test_hsprinter()
...
#missingreport (df, flow_list = flowlist, rainfall_list=rflist, output = 'output_flaw.png', printfig = True, output_flaw = output_flaw, dpi = 600, figsize=(20,10))
#test_missingreport()
...
#df = getts(fileinput_sep, index_col = 'Datahora', indexparse = ['Datahora'], separator = ',', limits = limits )
#sep_straight(df,event_duration, FLOWFieldName = 'Vazao', output = saida)
#test_sep_straight()
